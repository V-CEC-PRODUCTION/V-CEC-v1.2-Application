...
@LazySingleton(as: SignInGoogleService)
class SignInGoogleRepo extends SignInGoogleService {
  
  @override
  Future<Either<MainFailure, bool>> getAccessToken(String email) async {
    try {
      final response = await Dio(BaseOptions(contentType: 'application/json'))
          .post('${baseUrl}users/auth/sign-up/google/', data: {'email': email});
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        print(EmailModel.instance.accessToken);
        print(EmailModel.instance.refreshToken);
