// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'indeventscubit_cubit.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

/// @nodoc
mixin _$IndEventsState {
  IndEventsModel? get indEvents => throw _privateConstructorUsedError;
  bool get isLoading => throw _privateConstructorUsedError;
  Option<Either<MainFailure, IndEventsModel>> get isFailureOrSuccess =>
      throw _privateConstructorUsedError;

  @JsonKey(ignore: true)
  $IndEventsStateCopyWith<IndEventsState> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $IndEventsStateCopyWith<$Res> {
  factory $IndEventsStateCopyWith(
          IndEventsState value, $Res Function(IndEventsState) then) =
      _$IndEventsStateCopyWithImpl<$Res, IndEventsState>;
  @useResult
  $Res call(
      {IndEventsModel? indEvents,
      bool isLoading,
      Option<Either<MainFailure, IndEventsModel>> isFailureOrSuccess});
}

/// @nodoc
class _$IndEventsStateCopyWithImpl<$Res, $Val extends IndEventsState>
    implements $IndEventsStateCopyWith<$Res> {
  _$IndEventsStateCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? indEvents = freezed,
    Object? isLoading = null,
    Object? isFailureOrSuccess = null,
  }) {
    return _then(_value.copyWith(
      indEvents: freezed == indEvents
          ? _value.indEvents
          : indEvents // ignore: cast_nullable_to_non_nullable
              as IndEventsModel?,
      isLoading: null == isLoading
          ? _value.isLoading
          : isLoading // ignore: cast_nullable_to_non_nullable
              as bool,
      isFailureOrSuccess: null == isFailureOrSuccess
          ? _value.isFailureOrSuccess
          : isFailureOrSuccess // ignore: cast_nullable_to_non_nullable
              as Option<Either<MainFailure, IndEventsModel>>,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$IndEventsStateImplCopyWith<$Res>
    implements $IndEventsStateCopyWith<$Res> {
  factory _$$IndEventsStateImplCopyWith(_$IndEventsStateImpl value,
          $Res Function(_$IndEventsStateImpl) then) =
      __$$IndEventsStateImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {IndEventsModel? indEvents,
      bool isLoading,
      Option<Either<MainFailure, IndEventsModel>> isFailureOrSuccess});
}

/// @nodoc
class __$$IndEventsStateImplCopyWithImpl<$Res>
    extends _$IndEventsStateCopyWithImpl<$Res, _$IndEventsStateImpl>
    implements _$$IndEventsStateImplCopyWith<$Res> {
  __$$IndEventsStateImplCopyWithImpl(
      _$IndEventsStateImpl _value, $Res Function(_$IndEventsStateImpl) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? indEvents = freezed,
    Object? isLoading = null,
    Object? isFailureOrSuccess = null,
  }) {
    return _then(_$IndEventsStateImpl(
      indEvents: freezed == indEvents
          ? _value.indEvents
          : indEvents // ignore: cast_nullable_to_non_nullable
              as IndEventsModel?,
      isLoading: null == isLoading
          ? _value.isLoading
          : isLoading // ignore: cast_nullable_to_non_nullable
              as bool,
      isFailureOrSuccess: null == isFailureOrSuccess
          ? _value.isFailureOrSuccess
          : isFailureOrSuccess // ignore: cast_nullable_to_non_nullable
              as Option<Either<MainFailure, IndEventsModel>>,
    ));
  }
}

/// @nodoc

class _$IndEventsStateImpl implements _IndEventsState {
  const _$IndEventsStateImpl(
      {required this.indEvents,
      required this.isLoading,
      required this.isFailureOrSuccess});

  @override
  final IndEventsModel? indEvents;
  @override
  final bool isLoading;
  @override
  final Option<Either<MainFailure, IndEventsModel>> isFailureOrSuccess;

  @override
  String toString() {
    return 'IndEventsState(indEvents: $indEvents, isLoading: $isLoading, isFailureOrSuccess: $isFailureOrSuccess)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$IndEventsStateImpl &&
            (identical(other.indEvents, indEvents) ||
                other.indEvents == indEvents) &&
            (identical(other.isLoading, isLoading) ||
                other.isLoading == isLoading) &&
            (identical(other.isFailureOrSuccess, isFailureOrSuccess) ||
                other.isFailureOrSuccess == isFailureOrSuccess));
  }

  @override
  int get hashCode =>
      Object.hash(runtimeType, indEvents, isLoading, isFailureOrSuccess);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$IndEventsStateImplCopyWith<_$IndEventsStateImpl> get copyWith =>
      __$$IndEventsStateImplCopyWithImpl<_$IndEventsStateImpl>(
          this, _$identity);
}

abstract class _IndEventsState implements IndEventsState {
  const factory _IndEventsState(
      {required final IndEventsModel? indEvents,
      required final bool isLoading,
      required final Option<Either<MainFailure, IndEventsModel>>
          isFailureOrSuccess}) = _$IndEventsStateImpl;

  @override
  IndEventsModel? get indEvents;
  @override
  bool get isLoading;
  @override
  Option<Either<MainFailure, IndEventsModel>> get isFailureOrSuccess;
  @override
  @JsonKey(ignore: true)
  _$$IndEventsStateImplCopyWith<_$IndEventsStateImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
